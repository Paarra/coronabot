    ...
    async def on_message(self, message):
        ...
        if message.content.startswith('*'):
            if message.content == '*help':
                ...
                embed.add_field(name="*cure [user]", value="Spend 200 ☣️ to cure an infected target user.")
                ...
                for member in message.guild.members:
                    if check_user(member.id, 'state') == 'healthy':
                        healthy.append([member.name, str(check_user(member.id, 'bucks')), str(check_user(member.id, 'coughs'))])
                        healthy_money += int(check_user(member.id, 'bucks'))
                    else:
                        infected.append([member.name, str(check_user(member.id, 'bucks')), str(check_user(member.id, 'coughs'))])
                        infected_money += int(check_user(member.id, 'bucks'))
                stat_description = '__**Healthy**__ (nCoV-Bucks☣️ / Coughs🤧)'
                for person in healthy:
                    stat_description = stat_description + '\n' + person[0] + '   **(' + person[1] + '️/' + person[2] + ')**'
                stat_description = stat_description + '\n\n__**Infected**__ (nCoV-Bucks☣️ / Coughs🤧)'
                ...
                if datetime.datetime.now() - parser.parse(check_user(message.author.id, 'last_collection')) >= datetime.timedelta(hours=6):
                    ...
                    time_left = str(parser.parse(check_user(message.author.id, 'last_collection')) + datetime.timedelta(hours=6) - datetime.datetime.now())
                    time_left = time_left.split(':')
                    time_left[2] = str(round(float(time_left[2])))
                    embed = discord.Embed(title="Collection Failed",
                                          description="You have collected nCoV-Bucks in the past 6 hours.\n"
                                                      "Next collection in **" + time_left[0] + " hours, " + time_left[1] + ' minutes, ' + time_left[2] + ' seconds**.',
                                          color=0xff0000)
                    await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                    ...
                    if int(check_user(message.author.id, 'bucks')) >= 50:
                        ...
                        embed = discord.Embed(title="Purchase Successful",
                                              description='You bought item 1 from the Shop (Healthy).\n'
                                                          'You now have **' + str(check_user(message.author.id, 'bucks')) +'** ☣️.',
                                              color=0x00ff00)
                        await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                    else:
                ...
                if check_user(message.author.id, 'state') == 'healthy':
                    if int(check_user(message.author.id, 'bucks')) >= 1000:
                        change_file(message.author.id, 'immunity', 95)
                ...
                if int(check_user(message.author.id, 'coughs')) > 0:
                    if check_user(message.author.id, 'state') == 'infected':
                                ...
                                if randint(1, 100) > int(check_user(cough_id, 'immunity')):
                                    ...
                                    embed = discord.Embed(title="Infected!",
                                                          description='**' + message.author.name + "** infected **" + member + '**. \n **' + member + "**'s immunity was not strong enough.\n"
                                                                                                                                                      "You now have **" + str(check_user(message.author.id, 'coughs')) +"** coughs️.",
                                                          color=0x00ff00)
                                    await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                                    ...
                                    embed = discord.Embed(title="Close Call!",
                                                          description='**' + message.author.name + "** coughed on **" + member + '**. \n **' + member + "**'s " + str(
                                                              check_user(cough_id, 'immunity')) + "% immunity protected them from infection, for now...\n"
                                                                                                  "You now have **" + str(check_user(message.author.id, 'coughs')) + "** cough(s)️.",
                                                          color=0xff0000)
                                    await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                        ...
                        change_file(message.author.id, 'coughs', int(check_user(message.author.id, 'coughs')) - 1)
                        embed = discord.Embed(title="Cough Wasted",
                                              description='You are healthy, coughing on target has no effect.\n'
                                                          'You now have **' + str(check_user(message.author.id, 'coughs')) +'** cough(s)️.',
                                              color=0xff0000)
                        await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                else:
                ...
                cure_id = int(''.join(list(message.content)[-19:-1]))
                cure_person = str(message.guild.get_member(cure_id))
                if check_user(message.author.id, 'state') == 'healthy':
                    if check_user(cure_id, 'state') == 'infected':
                        if int(check_user(message.author.id, 'bucks')) >= 200:
                            change_file(message.author.id, 'bucks', int(check_user(message.author.id, 'bucks')) - 200)
                            embed = discord.Embed(title="Cured!",
                                                  description='**' + message.author.name + "** cured **" + cure_person + '**.\n'
                                                                                                                         'You now have **' + str(check_user(message.author.id, 'bucks')) +'** ☣️.',
                                                  color=0x00ff00)
                            await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                            change_file(cure_id, 'state', 'healthy')
                        else:
                            embed = discord.Embed(title="Cure Failed",
                                                  description='You do not have enough money, brokeass.',
                                                  color=0xff0000)
                            await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                        ...
                        change_file(message.author.id, 'bucks', int(check_user(message.author.id, 'bucks')) - 200)
                        embed = discord.Embed(title="Cure Wasted",
                                              description='Target is still healthy, you can only cure infected targets.\n'
                                                          'You now have **' + str(check_user(message.author.id, 'bucks')) +'** ☣️.',
                                              color=0xff0000)
                        await message.channel.send('{0.author.mention}'.format(message), embed=embed)
                else:
                    embed = discord.Embed(title="Cure Failed",
                                          description='You are infected, you can only cure people when you are healthy.',
                                          color=0xff0000)
                    await message.channel.send('{0.author.mention}'.format(message), embed=embed)
